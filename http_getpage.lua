...
local tuple2 = require'glue'.memoize(function(a, b) return {} end)

http.USERAGENT = 'Mozilla/5.0 (Windows NT 5.1)'
...
local function req_filename(req)
	...
	local dir = mkdirs(cache_dir, subdir)
	return dir .. filename, subdir
end
...
function getpage(req, on_complete, on_error)
	...
	for i = 1, 10 do
		loop.newthread(function()
			for i = 1, 6 do
				...
				local body, code, headers = download_page(url)
				if body then
